...
namespace MozartCS
{
  public partial class frmListeCliGIDTObj : Form
  {
    ...
    public frmListeCliGIDTObj() { InitializeComponent(); }

    /* OK --------------------------------------------------------------------------------------- */
    private void frmListeCliGIDTObj_Load(object sender, EventArgs e)
    {
      try
      {
        ...
        strRepImage = Utils.RechercheParam("REP_MEDIA") + @"GIDTCLI\";

        string sSql = "SELECT * FROM api_v_GIDTCLI_ListeObjet WHERE NCLINUM = " + miNumClient + " AND NSITNUM = " + miNumSite + " ORDER BY VSITNOM, VNIV1, VNIV2";
    ...
    //  rsObj.Open "SELECT * FROM api_v_GIDTCLI_ListeObjet WHERE NCLINUM = " & mlNumClient & " AND NSITNUM = " & mlNumSite & " order by vsitnom,vniv1,vniv2", cnx, adOpenDynamic, adLockOptimistic
    //  InitGrid
    //  mRepertoireDoc = RechercheParam("REP_MEDIA") + "GIDTCLI\"
    ...
    private void InitGrid()
    {
      ...
      Grid.AddColumn("Durée", "NOBJDUREEINTER", 1000);
      Grid.AddColumn("Durée", "NOBJMONTANTINTER", 1000);
    ...
    //  Grid.AddColumn "Contrat", "VTYPECONTRAT", 1800, , , , , , True
    ...
    //Public Sub ChargerImages(lObjet As Long, frm As Form, ByRef iNbImg As Integer, mRepertoireDoc As String)
    ...
    //  
    //  ' Recherche et affichage des images de cet objet
    //  rsImg.Open "SELECT * from TFICOBJCLI where NOBJET = " & lObjet, cnx
    //  
    ...
    //  
    //errH:
    //  ShowError "ChargerImages dans frmListeCliGidtObj"
    //End Sub
    ...
    private void ChargementImage(PictureBox imgCtl, string sFichier, long iFichier)
    {
      ...
      else
      {
        ...
        Image image = GetCopyImage(sFichier);
        imgCtl.Image = image;
    ...
    private void cmdOK_Click(object sender, EventArgs e)
    {
      ...
      {
        Cursor.Current = Cursors.WaitCursor;
        string sSql = $"UPDATE TGIDTOBJCLI set VOBJMAR = '{txtMarque.Text.Replace("'", "''")}', VOBJREF = '{txtRef.Text.Replace("'", "''")}', " +
    ...
    //  On Error GoTo handler
    //
    //  sSQL = "update TGIDTOBJCLI set VOBJMAR = '" & Replace(txtMarque.Text, "'", "''") & "', VOBJREF = '" & Replace(txtRef.Text, "'", "''") & "',"
    ...
    //  cnx.Execute "exec api_sp_SupprimerMedia " & Mid(Image1(Shape1.Tag).Tag, InStrRev(Image1(Shape1.Tag).Tag, "|") + 1) & ",'GIDTCLI'"
    ...
    /* Inutile --------------------------------------------------------------------------------------- */
    //'Private Sub cmdImpGidt_Click()
    //'Dim TdbGlobal(0 To 1, 0 To 1) As Variant
    //'Dim sSql As String
    //'
    //'    sSql = "SELECT api_v_GIDTCLI_ListeObjet.*, VCLINOM FROM api_v_GIDTCLI_ListeObjet, tcli" _
    //'        & " WHERE api_v_GIDTCLI_ListeObjet.NCLINUM = " & mlNumClient _
    //'        & " AND VSITNOM = '" & rsObj("VSITNOM") & "'" _
    //'        & " and api_v_GIDTCLI_ListeObjet.nclinum = tcli.nclinum"
    //'
    //'    ImprimerFichier "\modeles\TStockSite_Gidt.rtf", _
    //'                    "TStockSite_Gidt_Out.rtf", TdbGlobal(), sSql
    //'
    //'End Sub
  }
}