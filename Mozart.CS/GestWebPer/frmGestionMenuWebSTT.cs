...
namespace MozartCS
{
  public partial class frmGestionMenuWebSTT : Form
  {
    ...
    public frmGestionMenuWebSTT()
    {
      InitializeComponent();
    }

    private void frmGestionMenuWebSTT_Load(object sender, System.EventArgs e)
    {
      try
      {
    ...
    /* OK ---------------------------------------------------------------------*/
    private void cmdPetP_Click(object sender, System.EventArgs e)
    {
      Cursor.Current = Cursors.WaitCursor;
      new frmCopyProfil()
      {
    ...
    //Private Sub cmdPetP_Click()
    //  frmCopyProfil.miNumClient = miNumClient
    //  frmCopyProfil.strType = strType
    //  frmCopyProfil.Show vbModal
    //  FillTreeView
    //End Sub
    ...
    private void FillTreeView()
    {
      try
      {
        ...
        string sSql = $"select MenuAdminLibelle 'VMENULIB', MenuParent 'NMENUPARENT', id 'NMENUNUM', CDROITVAL from tw2droit inner join dbo.TW2MENU " +
                      $"on tw2droit.IDMENU = TW2MENU.id Where NSTFNUM = {miNumClient} AND MenuAdminVisible = 'O' ORDER BY NMENUPARENT, VMENULIB";
    ...
    //  
    //  sSQL = "select  MenuAdminLibelle 'VMENULIB', MenuParent 'NMENUPARENT', id 'NMENUNUM', CDROITVAL "
    //  sSQL = sSQL & "from    tw2droit inner join dbo.TW2MENU on tw2droit.IDMENU=TW2MENU.id "
    //  sSQL = sSQL & "Where NSTFNUM = " & miNumClient
    //  sSQL = sSQL & " AND MenuAdminVisible = 'O' "
    //  sSQL = sSQL & " ORDER BY NMENUPARENT, VMENULIB "
    //        
    ...
    private void tvwDB_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
    {
      ...
      ModuleData.ExecuteNonQuery($"Update TW2DROIT SET CDROITVAL = '{cDroit}' Where NSTFNUM = {miNumClient} AND IDMENU = {e.Node.Name}");
    ...
    //  cnx.Execute "Update TW2DROIT SET CDROITVAL = '" & cDroit & "' Where NSTFNUM = " & miNumClient & " AND IDMENU = " & val(Node.Key)
    //
    //  Node.Image = cDroit
    //    
    //End Sub
    ...
    //  strQ = "SELECT  distinct VUTILOG From TUTI WHERE CUTICAT = '" + strType + "' AND NUTINUM = " & miNumClient & " ORDER BY VUTILOG"
    //          
