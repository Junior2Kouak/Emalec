...
namespace MozartCS
{
    public partial class frmMethodes_Admin_Equipement_Fiche_Detail : Form
    {
        ...
        public frmMethodes_Admin_Equipement_Fiche_Detail(Int32 c_NID_EQUIPEMENT_FICHE)
        {
            InitializeComponent();
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            int nbrowWithTypeChampCodeBarre = 0; //ce champ permet de save le nb de champ en type code barre, il ne peut y avoir qu'un seul code barre par fiche
            ...
            for (int i = 0; i < GVAdmin_Equipement_Fiche_Items.DataRowCount; i++)
            {
                ...
                if (GVAdmin_Equipement_Fiche_Items.GetRowCellValue(i, "NID_EQUIPEMENT_FICHE_CHAP") != null 
                        && (int)GVAdmin_Equipement_Fiche_Items.GetRowCellValue(i, "NID_EQUIPEMENT_FICHE_CHAP") ==0 && oCEQUIPEMENT_FICHE.NID_EQUIPEMENT_FICHE != 0)
                {
                ...
                //test le code barre
                if ((int)GVAdmin_Equipement_Fiche_Items.GetRowCellValue(i, "NID_EQUIPEMENT_FICHE_TYPE_CHAMP") == 8)
                {
                    nbrowWithTypeChampCodeBarre = nbrowWithTypeChampCodeBarre + 1;
                    if (nbrowWithTypeChampCodeBarre > 1)
                    {
                        MessageBox.Show("Une fiche d'équipements ne peut contenir qu'un seul champ de type CODE BARRE", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }                                
        ...
        private void BtnSupprimer_Click(object sender, EventArgs e)
        {
            ...
            if (CEQUIPEMENT_FICHE_TYPE_CHAMP.ListTypeChampObligatoire().Contains((int)currentRow["NID_EQUIPEMENT_FICHE_TYPE_CHAMP"]))
            {
        ...
        private void GVAdmin_Equipement_Fiche_Items_ShowingEditor(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            //si editor champ obligatoire et type champ code barre, on bloque la modification du champ obligatoire
            if (GVAdmin_Equipement_Fiche_Items.FocusedColumn.FieldName == "BEQUIPEMENT_FICHE_ITEM_OBLIG" &&
                (int)view.GetRowCellValue(GVAdmin_Equipement_Fiche_Items.FocusedRowHandle, "NID_EQUIPEMENT_FICHE_TYPE_CHAMP") == 8)
            {
                e.Cancel = true;
                return;
            }
        ...
        private void GVAdmin_Equipement_Fiche_Items_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (e.Column.FieldName == "NID_EQUIPEMENT_FICHE_TYPE_CHAMP" && e.Value != DBNull.Value)
            {
                ...
                //si type champ code barre alors on coche automatiquement obligatoire
                if ((int)e.Value == 8)
                {
                    view.SetRowCellValue(e.RowHandle, "BEQUIPEMENT_FICHE_ITEM_OBLIG", true);
                    GVAdmin_Equipement_Fiche_Items.PostEditor();
                    GVAdmin_Equipement_Fiche_Items.UpdateCurrentRow();
                    return;
                }
                //si type de champ n'est pas une liste alors on initialise la combo
                if (!CEQUIPEMENT_FICHE_TYPE_CHAMP.LoadListeTypeChampInListe().Contains((int)e.Value))
                {
