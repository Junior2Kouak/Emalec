...
namespace MozartCS
{
	public partial class frmInventaireEquipementSite : Form
	{
		...
		public frmInventaireEquipementSite()
		{
			InitializeComponent();
		}

		private void frmInventaireEquipementSite_Load(object sender, EventArgs e)
		{
			try
			{
		...
		{
			ModuleData.LoadData(dt, $"EXEC [api_sp_InvEquip_DetailsListeContratEquipement_By_Site] {nsitnum}, {!bArchives}");
		...
		private void LoadDataDetails()
		{
			...
			ModuleData.LoadData(dtDetails, $"EXEC [api_sp_InvEquip_DetailsBySite] {nsitnum},{row["NTYPECONTRAT"]}, {row["NIDEQUIPEMENT"]}, {row["NID_EQUIPEMENT_FICHE"]}, {!bArchives}");
			...
			dtDetails.Columns["BEQUIPEMENT_A_CORRIGER"].ReadOnly = false;
		...
		private void GestAffichage()
		{
			...
			if (bArchives)
			{
				this.Text = LblTitre.Text = $"{vclinom} : Inventaire des équipements inactifs du site {vsitnom}";
				BtnArchivesEquipement.Text = "Voir les actifs";

			}
			else
			{
				this.Text = LblTitre.Text = $"{vclinom} : Inventaire des équipements actifs du site {vsitnom}";
		...
		{
			new frmInventaireEquipementNew_SelectTypeEquipement()
			{
				NSITNUM = nsitnum
			}.ShowDialog();
		...
		private void PVTG_Details_CustomDrawCell(object sender, PivotCustomDrawCellEventArgs e)
		{
		...
		private void BtnExportXLS_Click(object sender, EventArgs e)
		{
            ...
            List<int> LstInvSitesSelected = new List<int>(){MozartDatabase.ExecuteScalarInt($"EXEC [api_sp_InvEquip_GetNSIT_INV_IDByNSITNUM] {nsitnum}")};           
